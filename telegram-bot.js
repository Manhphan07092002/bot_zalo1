...
} = require('./services/callback-handlers');
...
bot.on('callback_query', async (query) => {
  ...
  try {
    ...
    if (await handleQuoteListCallbacks({
      data, query, chatId, userId, bot,
      ensureAdmin, buildQuoteListView, isAdminUser,
      getRecentQuotes, getQuotesByUser, findQuotesByKeyword,
      sendQuotePicker
    })) return;

    if (await handleQuoteActionCallbacks({
